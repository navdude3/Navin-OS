...
static char shift_let_num [OPTION_SIZE] = {')', '!', '@', '#', '$', '%', '^', '&', '*', '(', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 
...
0x26, 0x32, 0x31, 0x18, 0x19, 0x10, 0x13, 0x1F, 0x14, 0x16, 0x2F, 0x11, 0x2D, 0x15, 0x2C, 0x0C, 0x0D, 0x1A, 0x1B, 0x27, 0x29, 0x33, 0x34, 0x35, 0x39, 0x2B, 0x28, 0x0F};

static lshift = 0;
static rshift = 0;
static caps_en = 0;
static shift_en = 0;
static capital_letters = 0;
/* 
* keyboard_init 
...
void shift_check(int key) {
    ...
    else if(key == RSHIFT_PRESS){
        rshift = 1;
    }
    else if(key == LSHIFT_RELEASE){
        lshift = 0;
    }
    else if(key == RSHIFT_RELEASE){
        rshift = 0;
    }
    else if(key == CAPS_PRESSED){
        caps_en = 1;
    }
    else if(key == CAPS_RELEASED){
        caps_en = 0;
    }
}
